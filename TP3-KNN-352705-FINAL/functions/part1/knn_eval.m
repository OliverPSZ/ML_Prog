...
for i=1:length(params.k_range)                      % Loop to evaluate different values of k
    ...
    y_est = knn(X_train,  y_train, X_test, params); % Run KNN to get the estimated labels
    acc_curve(i) = accuracy(y_test, y_est);         % Get and save values of accuracy with evaluated k
