...
switch(type)
    case 'L1'
        d = sum(abs(x_1-x_2));              % Get the Manhattan distance (L1 norm)

    case 'L2'
        d = sqrt(sumsqr(abs(x_1-x_2)));     % Get the Euclidean distance (L2 norm)

    case 'LInf'
        d = max(abs(x_1-x_2));              % Get the Maximum distance between dimensions (Linf norm)
