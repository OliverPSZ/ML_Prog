...
[Priors, Mu, Sigma] = gmmInit(X, params);                       % Initialize values
logl0 = gmmLogLik(X, Priors, Mu, Sigma);                        % Get loglikelihood from initial values
iter = 0;                                                       % Initialize iteration counter
tol = 0.00001;                                                  % Set tolerance

for i=1:params.max_iter
    Pk_x = expectation_step(X, Priors, Mu, Sigma, params);      % Expectation step  
    [Priors,Mu,Sigma] = maximization_step(X, Pk_x, params);     % Maximization step
    logl = gmmLogLik(X, Priors, Mu, Sigma);                     % Get loglikelihood of the iteration
    iter = iter+1;                                              % Increase iteration counter
    if logl-logl0 <= tol                                        % Evaluate if the loglikelihood has converged
        break;                                                  % Exit loop if it has converged
    end
    logl0 = logl;                                               % Set previous logl 
end 
