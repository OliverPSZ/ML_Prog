...
unique_cards = unique(reshape(data,[1,size(data,1)*size(data,2)]));      % Find the unique cards in the dataset

for i=1:size(unique_cards,2)
    for j=1:size(data,2)
        X(i,j) = sum(ismember(data(:,j),unique_cards(i)));               % Obtain the number of times the card is in each deck
    end
end
