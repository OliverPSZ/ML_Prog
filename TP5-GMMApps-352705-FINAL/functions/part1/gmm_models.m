...
labels = unique(Ytrain);                % Get unique classes from Ytrain
for i=1:length(labels)
    idx = find(Ytrain==labels(i));      % Get indexes where the i-th class appears in Ytrain
    [models(i).Priors, models(i).Mu, models(i).Sigma] = gmmEM(Xtrain(:,idx), params);   % Get GMM of the selected data from the class
end
